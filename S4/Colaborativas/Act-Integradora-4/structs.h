...
struct Registro {
    string value; // Formato: Sep 17 00:54:39 33.213.148.82:2528 137.169.193.233:3170 53 Failed password for admin
    Date date; // Formato: Oct 9
    Time time; // Formato de la hora: 10:32:24
    IpAddress origen; // Formato: 63.24.243.11:6010
    IpAddress destino; // Formato: 63.24.243.11:6010
    int peso; // Peso del registro, que es dado el registro; Formato: 53
    string mensaje; // Formato: Failed password for illegal user guest
    ...
    Registro(string valor) {
        ...
        // Extraer los componentes del string
        string mes, dia, hora, ipOrigen, ipDestino, pesoStr;
        
        // Leer mes y día para la fecha
        ss >> mes >> dia;
        ...
        // Leer las IPs
        ss >> ipOrigen >> ipDestino;
        origen.set(ipOrigen);
        destino.set(ipDestino);
        
        // Leer el peso
        ss >> pesoStr;
        peso = stoi(pesoStr);
        
        // Leer el mensaje (resto de la línea)
        getline(ss, mensaje);
